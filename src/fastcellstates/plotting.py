"""
Dendrograms of a hierarchy DataFrame (``Summary.hierarchy()`` /
``Cluster.get_cluster_hierarchy()``): a scipy version always available, and an
ete3 one if the optional ``ete3`` package is installed (conda-forge only; its
PyPI build is broken).  Needs the ``plot`` extra; loaded lazily by
``fastcellstates.__getattr__``, not imported by the package itself.
"""
...
from .analysis import clusters_from_hierarchy, get_scipy_hierarchy, hierarchy_to_newick

USE_ETE = True
try:
    from ete3 import NodeStyle, Tree, TreeStyle  # type: ignore[import-untyped]
except ImportError:
    USE_ETE = False


def plot_hierarchy_scipy(hierarchy_df, n_groups=2, dflt_color="#808080", colors=None, **kwargs):
    """
    ...
    **kwargs :
    ...
    cluster_colors = {i: mcolors.to_hex(c) for i, c in zip(np.unique(clusters), colors)}
    # Color mapping
    ...
    R = dendrogram(
        Z,
        color_threshold=None,  # Z[-n_groups+1, 2])
...
if USE_ETE:
    ...
    def plot_hierarchy_ete3(
        ...
        h_clusters = clusters_from_hierarchy(
            hierarchy_df, cluster_init=cellstate_names, steps=-n_groups + 1
        )
        cluster_names = np.unique(h_clusters)

        if colors is None:
            colors = plt.cm.hsv(np.linspace(0, 1, n_groups + 1))[:-1]
        color_map = {cn: mcolors.to_hex(cl) for cn, cl in zip(cluster_names, colors)}
        ...
        for cn in cluster_names:
            ...
            leaf_names = all_leaf_names[(h_clusters == cn)]
            if len(leaf_names) == 1:
